...
evsDefaults = {'individual': 'compositeFixedProbabilities', 'evolver': 'cluneSimplifiedMorphologyControlIndividuals', 'communicator': 'chunkedUnixPipe',
               ...
               'logParetoFront': 'yes', 'logBestIndividual': 'yes', 'logParetoFrontKeepAllGenerations': 'yes', 'logParetoFrontPeriod': 50, 'logParetoSize': 'yes',
...
def processResults(experiment):
	...
	tplt.plotComputationVariableAgainstParameter(experiment, 'mmmmd', generateMinMMMDistTimeSlices, xparam,
                                       ...
                                       forcedXLabelPos=[1,-0.06], forcedYLabelPos=[0.05,1.02], legendLocation=3, marker='*')
	...
	tplt.plotComputationVariableAgainstParameter(experiment, 'error', generateFitnessTimeSlices, xparam,
	                                     fieldNames=fieldNames, transform=lambda x: -1.*x, yscale='lin', xlabel=xlabel, ylabel=r'$\log_{10} E$', strips='conf95', 
                                       forcedXLabelPos=[1,-0.06], forcedYLabelPos=[0.05,1.02], legendLocation=3, marker='*')
